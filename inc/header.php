      ...
      <div class="d-flex">
        <?php 
          if(isset($_SESSION['login']) && $_SESSION['login']==true)
          {
            ...
            $review_count_q = "SELECT COUNT(*) AS `count` FROM `rating_review` WHERE `user_id`=? AND `seen`=?";
            $contact_count_q = "SELECT COUNT(*) AS `count` FROM `user_queries` WHERE `email`=? AND `seen`=?";
            $review_count_r = mysqli_fetch_assoc(select($review_count_q, [$_SESSION['uId'],1], 'ii'));
            $contact_count_r = mysqli_fetch_assoc(select($contact_count_q, [$_SESSION['uEmail'],1], 'si'));
            $notification_count = ($review_count_r['count'] ?? 0) + ($contact_count_r['count'] ?? 0);
            $notification_badge = '';
            if($notification_count){
              $notification_badge = "<span class='position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger'>$notification_count</span>";
            }
            $path = USERS_IMG_PATH;
            echo<<<data
              <div class="d-flex align-items-center me-2">
                <a href="notifications.php" class="btn btn-outline-dark shadow-none me-2 position-relative" style="border-radius:var(--radius-sm);width:44px;height:44px;display:flex;align-items:center;justify-content:center;">
